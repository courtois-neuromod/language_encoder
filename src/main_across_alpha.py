...
for set in ["bourne"]:
    ...
    for training_season in friends:


        print(f"Running training and testing for {data_config.subject_id} on data: {training_season} and figures, life, {set}")
        train_friends_set, val_friends_set, train_movie10_set, val_movie10_set, train_movie10_data_list, val_movie10_data_list= split_across_dataset(
            data_config, set, training_season, friends, movie10) 
        ...
        x_train = get_hrf_convolved_embeddings_accross_dataset(data_config, train_friends_set, train_movie10_set, length_train)
        ...
        for val_run in val_friends_set + val_movie10_set:
            if val_run in val_friends_set: 
