...
from features import DesignMatrixBuilder, DimensionReductor, FeatureSelector
from metrics import get_metric
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler
from utils_a import get_linearmodel
...
class Encoder(object):
    ...
    def fit(self, X, y, groups=None, gentles=None, nscans=None):
        ...
        print(f"len_gentles: {len(gentles)}")
        print(f"len_gentles: {len(X)}")
        ...
        fmri_pipe = Pipeline(
            [
                (
                    "selector",
                    FeatureSelector(),
                ),  # Select non nan and non-constant values
        ...
        my_data = features_pipe.fit_transform(X)
        print(f"my_data: {len(my_data)}")
        encoding_pipe.fit(my_data, fmri_pipe.fit_transform(y))
    ...
    def predict(self, X):
        ...
        """
        logging.warning(
            f"X should not be process. It should be the embeddings derived from the generator model: before building the design matrix."
        )
    ...
    def eval(self, Y_predicted, Y, metric_name="r"):
        ...
        """
        logging.warning(
            f"Y should not be process. It should be the fMRI data before voxels selection, scaling and dimension reduction."
        )
