...
from utils_a import get_reduction_method
...
class FeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None, mask=None):
        ...
        """
        logging.info(
            f"Identifying unuseful features: pre-selected features, constant features and features having nan..."
        )
        std = X.std(0)
    ...
    def transform(self, X):
        ...
        if self.method == "hrf":
            ...
            for i, group in enumerate(self.groups):
                print(f"iteration: {i}")
                if any(idx >= X.shape[0] or idx < 0 for idx in group):
                    logging.error(f"Invalid index in group {i}: {group}")
                ...
                assert output[-1].shape[0] == nscan, (output[-1].shape[0], nscan)
